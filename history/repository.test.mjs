...
function healthySchema(overrides = {}) {
  return {
    ...
    keeper_durable_signed_envelope_migration_valid: true,
...
test('history health requires schema v10, one active Bradbury V8, and complete projections', async () => {
  ...
  assert.equal(calls[0].params.length, 17);
  ...
  assert.match(calls[0].text, /version = 4[\s\S]*version = 5[\s\S]*version = 6[\s\S]*version = 7[\s\S]*version = 8[\s\S]*version = 9[\s\S]*version = 10/);
