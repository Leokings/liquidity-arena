...
Migrations 006–010 extend that append-only journal without rewriting migrations 001–005. Migration 006 adds the receipt-proven two-slot `ACCEPTED` handoff pipeline and its subject/lineage constraints. Migration 007 adds immutable evidence for either a definite local pre-spawn failure or an operator-audited no-broadcast abandonment; keeper admission permits only the deterministic attempt-two child. Migration 008 removes the legacy constraint that blocked that abandonment state. Migration 009 extends audited pre-hash recovery to exact `create_epoch` operations with an `EPOCH_UNKNOWN` post-state proof and restores the health-pinned finalized-state constraint. Migration 010 persists one private signed envelope before broadcast, keeps raw bytes out of public operation responses, and binds the outer EVM hash, nonce, finalized receipt outcome, and emitted inner transaction identity before journal submission advances.

Public history exposes only V8 deployments, epochs, proofs, and payouts. Health requires exact contract/role/factory/schema identity, journal schema version 10 with no unknown later migration, complete epoch/payout projections, and verified payout-stage evidence.
