...
class LiquidityArenaV6(gl.Contract):
    owner: Address
    treasury: Address
    ...
    def __init__(self, treasury: Address):
        ...
        if _address_text(treasury) == "0x" + ("0" * 40):
    ...
    def create_epoch(
        ...
        min_stake_atto: u256,
        max_stake_per_wallet_atto: u256,
    ) -> None:
        self._require_zero_value()
        self._require_owner()
        epoch_end = int(epoch_end_timestamp)
        minimum_stake = int(min_stake_atto)
        maximum_wallet_stake = int(max_stake_per_wallet_atto)
        now = _now_epoch()
        ...
        if now >= wager_opens:
            _expected("EPOCH_NOTICE", "Epoch must be created before its wager window opens")
        if epoch_end - now > MAX_SCHEDULE_AHEAD_SECONDS:
    ...
    def _claim_quote(
        ...
        if not claimed and stake > 0:
            if mode in REFUND_SETTLEMENT_MODES:
                ...
                paid = int(
                    self.objective_paid_atto.get(objective_key, u256(0))
                )
        ...
        return {
            ...
            "eligible": not claimed and amount > 0,
            "claimed": claimed,
    ...
    def claim(self, epoch_end_timestamp: u256, objective: str) -> None:
        ...
        if amount > int(self.balance):
            _expected("ESCROW_BALANCE", "Contract balance is below the claim amount")
        ...
        _EOARecipient(sender).emit_transfer(value=u256(amount), on="finalized")

    @gl.public.write
    def withdraw_accrued_fees(self, amount_atto: u256) -> None:
        self._require_zero_value()
        ...
        if balance < liability or amount > balance - liability:
            _expected("FEE_SOLVENCY", "Withdrawal would touch player liabilities")

        self.accrued_platform_fees_atto = u256(accrued - amount)
        self.withdrawn_platform_fees_atto = u256(
            int(self.withdrawn_platform_fees_atto) + amount
        )
        _EOARecipient(self.treasury).emit_transfer(
            value=u256(amount),
            on="finalized",
        )
    ...
    def get_objective(self, epoch_end_timestamp: u256, objective: str) -> dict:
        ...
        record["remaining_payout_atto"] = int(record["payout_pool_atto"]) - paid
