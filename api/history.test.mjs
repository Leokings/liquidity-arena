...
test('history health requires exact Bradbury V8 configuration and schema-v8 integrity', async () => {
  const integrity = {
    checked: true,
    ready: false,
  ...
  const repository = {
    configured: true,
    async health() {
      return { configured: true, ready: false, schemaVersion: 8, integrity };
    },
  };
  ...
  assert.equal(payload.database.schemaVersion, 8);
