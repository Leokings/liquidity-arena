...
test('history health requires exact Bradbury V8 configuration and schema-v7 integrity', async () => {
  const integrity = {
    checked: true,
    ready: false,
  ...
  const repository = {
    configured: true,
    async health() {
      return { configured: true, ready: false, schemaVersion: 7, integrity };
    },
  };
  ...
  assert.equal(payload.database.schemaVersion, 7);
