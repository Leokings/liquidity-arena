...
test('history health requires exact Bradbury V8 configuration and schema-v9 integrity', async () => {
  const integrity = {
    checked: true,
    ready: false,
  ...
  const repository = {
    configured: true,
    async health() {
      return { configured: true, ready: false, schemaVersion: 9, integrity };
    },
  };
  ...
  assert.equal(payload.database.schemaVersion, 9);
